...
namespace SpaNSalon
{
    public partial class EmployeeLogin : System.Web.UI.Page
    {
        ...
        private void login()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("select username, password from empMst where username=@username AND password =@password;", conn);
                ...
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        Session["employeeID"] = dr.GetValue(0).ToString();
